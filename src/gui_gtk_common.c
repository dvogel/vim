 ...
 * implementations. The implementations of the functions can use preprocessor
 * conditions to implement different behavior. The function definitions should
 * be sharable across implementations though.
 */
 ...
 */
    guint
gui_gtk_timeout_add(int time, gboolean (*callback)(gpointer), int *flagp)
{
...
#  ifdef USE_GTK4
    // Ignore icon_size. In GTK4 actual pixel sizes are determined by themes
    // via CSS. Therefore the pixbuf data should not be constrained to a
    // specific pixel size.
    pixel_size = -1;
#  else
    switch (icon_size)
