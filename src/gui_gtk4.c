...
/*
 * Similar in function to XParseGeometry but with significant differences.
 * Only parses width and height. Ignores X and Y offsets. Missing components
 * are inidicated by a 0 value in out_width or out_height.
 */
    void
parse_geometry(const char *geom, long *out_width, long *out_height)
{
    const char *p = geom;
    long accum = 0;
...
gui_mch_open(void)
{
    ...
    if (gui.geom != NULL)
    {
	parse_geometry(gui.geom, &Columns, &Rows);
	limit_screen_size();
    }
...
#define GTK4_CLIPBOARD_SCRATCH_LEN 1024
    ...
    {
	semsg("Clipboard as formats: %s", gdk_content_formats_to_string(formats));
    }
...
menu_position_within_parent(vimmenu_T *menu, int *out_position)
{
    ...
    *out_position = position;
    ...
    static GtkWidget *
load_menu_iconfile(char_u *name)
{
